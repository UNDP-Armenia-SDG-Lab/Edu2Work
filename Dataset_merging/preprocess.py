...
def load_to_mongoDB(database, collection, dataframe): 
    '''Function loading dataframe to db's respective collection '''
    client = MongoClient()
...
#filling empty string with None
'''job_am=job_am.replace(r'^\s*$', np.nan, regex=True)
job_am=job_am.replace(" NA", np.nan)
job_am=job_am.where(job_am.notna(),None)'''
job_am=na_filler(job_am)
...
'''jobfinder=jobfinder.replace(r'^\s*$', np.nan, regex=True)
jobfinder=jobfinder.replace("-N/A", np.nan)
jobfinder=jobfinder.where(jobfinder.notna(),None)'''
jobfinder=na_filler(jobfinder)
...
'''worknet=worknet.replace(r'^\s*$', np.nan, regex=True)
worknet=worknet.replace("'None'", np.nan)
worknet=worknet.where(worknet.notna(),None)'''
worknet=na_filler(worknet)
#adding website name
...
#filling empty string with None
career_center2=na_filler(career_center2)

#CAREERCENTER's modified website

#filling empty string with None
careercenter=na_filler(careercenter)
...
#droping observations having NA in Job Content
